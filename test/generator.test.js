...
async function loadGeneratorModule() {
  ...
  try {
    return await import(distEntry.href);
  } catch (error) {
    const nodeError = error;
    if (nodeError && typeof nodeError === 'object' && 'code' in nodeError && nodeError.code === 'ERR_MODULE_NOT_FOUND') {
      const buildScript = new URL('../scripts/run-build.js', import.meta.url);
