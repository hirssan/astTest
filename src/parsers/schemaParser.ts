...
const rubyFallbackScriptPath = fileURLToPath(
	new URL("./rubyFallback.rb", import.meta.url),
);

type PrismParseFunction = (
	source: string,
...
async function loadPrismParser(): Promise<{
	...
	try {
		...
		const parseFn =
			(prismModule as { parse?: PrismParseFunction })?.parse ??
			(prismModule as { default?: { parse?: PrismParseFunction } })?.default
				?.parse;
...
function visitPrismAst(
	node: unknown,
	visitor: PrismVisitor,
...
function extractIdentifierName(node: unknown): string | undefined {
	...
	if (typeof (json as { name?: string }).name === "string")
		return (json as { name: string }).name;
	if (typeof (json as { value?: string }).value === "string")
...
function literalFromNode(node: unknown, source: string): unknown {
	...
	if (isRecord(json)) {
		...
		if (Array.isArray((json as { parts?: unknown[] }).parts)) {
			return (json as { parts: unknown[] }).parts
				.map((part) => literalFromNode(part, source))
				.join("");
		}

		if (Array.isArray((json as { elements?: unknown[] }).elements)) {
			return (json as { elements: unknown[] }).elements.map((element) =>
				literalFromNode(element, source),
			);
		}

		const locationCandidate = (json.location ?? json.loc ?? json) as
			| Record<string, unknown>
			| undefined;
		const startOffset =
			typeof locationCandidate?.startOffset === "number"
				? locationCandidate.startOffset
				: undefined;
		const endOffset =
			typeof locationCandidate?.endOffset === "number"
				? locationCandidate.endOffset
				: undefined;
...
function argumentsFromCall(callNode: unknown): unknown[] {
	...
	if (Array.isArray(json.arguments))
		return json.arguments.map((argument) => toJsonNode(argument));
	if (
		Array.isArray(
			(json.arguments as { arguments?: unknown[] } | undefined)?.arguments,
		)
	) {
		return (json.arguments as { arguments: unknown[] }).arguments.map(
			(argument) => toJsonNode(argument),
		);
	}
	if (Array.isArray(json.args))
		return json.args.map((argument) => toJsonNode(argument));
	return [];
}
...
function blockStatements(blockNode: unknown): unknown[] {
	...
	if (
		Array.isArray(
			(json.body as { statements?: unknown[] } | undefined)?.statements,
		)
	) {
		return (json.body as { statements: unknown[] }).statements;
	}
	if (
		Array.isArray((json.statements as { body?: unknown[] } | undefined)?.body)
	) {
...
function optionsFromNode(node: unknown, source: string): ColumnOptions {
	...
	return pairs.reduce<ColumnOptions>((acc, pair) => {
		...
		const tuple = Array.isArray(pairJson)
			? pairJson
			: Array.isArray(pair)
				? pair
				: undefined;
...
function createColumnsFromBlock(
	blockNode: unknown,
	source: string,
): ColumnDefinition[] {
	...
	for (const statement of statements) {
		...
		const methodName = extractIdentifierName(
			json.name ?? json.method ?? json.identifier,
		);
		if (!methodName) continue;

		if (methodName === "timestamps" || methodName === "t.timestamps") {
			columns.push(
				...TIMESTAMP_COLUMNS.map((column) => ({
					...column,
...
function extractTableFromBlock(
	blockNode: unknown,
	source: string,
	...
	const methodName = extractIdentifierName(
		callJson?.name ?? callJson?.method ?? callJson?.identifier,
	);
...
function parseWithRubyFallback(schemaContent: string): ParsedSchema {
	...
	const tables: TableDefinition[] = Array.isArray(payload.tables)
		? payload.tables
				.filter(
					(table): table is RubyFallbackTable & { name: string } =>
				...
				.map((table) => ({
					...
					columns: Array.isArray(table.columns)
						? table.columns
								...
								.map((column) => {
									const options = isRecord(column.options)
										? (column.options as ColumnOptions)
										: {};
									let enumName = column.enumName;
									if (!enumName && column.type === "enum") {
										const raw = (options.enum_type ??
											options.enum ??
											options.name) as unknown;
	...
	const enums: EnumDefinition[] = Array.isArray(payload.enums)
		? payload.enums
				.filter(
					(
						definition,
					): definition is Required<RubyFallbackEnum> & { name: string } =>
	...
	if (result.status !== 0) {
		errors.push(
			`Ruby フォールバックが終了ステータス ${result.status} で失敗しました。`,
		);
	}
